...
import { FileText, Edit2, Check, ChevronDown, ChevronUp, Loader2, AlertCircle } from "lucide-react"
...
function MetadataCard({ item, onApply }: MetadataCardProps) {
  ...
  const commitEdit = () => {
    ...
    Object.entries(draft).forEach(([k, v]) => {
      if (k === "mentioned_subjects") updated[k] = v.split(",").map((s) => s.trim()).filter(Boolean)
      else updated[k] = v
    })
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        item.applied ? "border-[#0052FF]/20 shadow-[0_4px_24px_rgba(0,82,255,0.08)]" : "border-[#E2E8F0]",
      )}
    >
      {item.applied && (
      ...
      <div className="flex items-center justify-between gap-3 px-5 py-4">
        <div className="flex min-w-0 items-center gap-3">
          <div
            ...
            style={{ background: "linear-gradient(135deg, #0052FF, #4D7CFF)", boxShadow: "0 4px 14px rgba(0,82,255,0.2)" }}
          >
            <FileText className="size-4 text-white" />
          </div>
          <span className="truncate font-mono text-xs font-medium text-[#0F172A]">{item.data_path}</span>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          {item.applied && (
            <span className="flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[11px] font-semibold text-white" style={{ background: "linear-gradient(to right, #0052FF, #4D7CFF)" }}>
              <Check className="size-3" /> Đã áp dụng
            </span>
          )}
          <button onClick={() => setExpanded((v) => !v)} className="rounded-lg p-1.5 text-[#64748B] hover:bg-[#F1F5F9]">
              ...
              {editing ? (
                <div className="flex flex-col gap-3">
                  {Object.entries(METADATA_LABELS).map(([key, label]) => (
                    <div key={key} className="flex items-start gap-3">
                      <span className="w-40 shrink-0 pt-1.5 text-xs font-medium text-[#64748B]">{label}</span>
                      <input
                        value={draft[key] ?? ""}
                ...
                <div className="flex flex-col gap-2">
                  {Object.entries(METADATA_LABELS).map(([key, label]) => {
                    ...
                    return (
                      <div key={key} className="flex gap-3 text-xs">
                        <span className="w-40 shrink-0 font-medium text-[#64748B]">{label}</span>
                  ...
                  <div className="flex justify-end gap-2 pt-2">
                    ...
                    {!item.applied && (
                      <button
                        onClick={() => onApply(item.data_path, item.light_metadata)}
                        className="flex items-center gap-1.5 rounded-xl px-4 py-2 text-xs font-semibold text-white"
                        style={{ background: "linear-gradient(to right, #0052FF, #4D7CFF)", boxShadow: "0 4px 14px rgba(0,82,255,0.2)" }}
                      >
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="mb-1 inline-flex items-center gap-2 rounded-full border border-[#0052FF]/30 bg-[#0052FF]/5 px-3 py-1">
            ...
            <span className="font-mono text-[11px] uppercase tracking-[0.15em] text-[#0052FF]">Bước 3 / 5</span>
          </div>
        ...
        {!loading && (
          <div className="shrink-0 text-right">
            ...
            <p className="text-2xl font-bold text-[#0052FF]">{appliedCount}<span className="text-sm font-normal text-[#64748B]">/{items.length}</span></p>
          </div>
        )}
      </div>
      ...
      {loading ? (
        <div className="flex items-center justify-center gap-3 rounded-2xl border border-[#E2E8F0] bg-white py-16">
          ...
          <span className="text-sm text-[#64748B]">Đang tải metadata từ OCR…</span>
        </div>
      ) : items.length === 0 ? (
        <div className="flex items-center justify-center gap-3 rounded-2xl border border-[#E2E8F0] bg-white py-16">
          <AlertCircle className="size-5 text-[#64748B]" />
          <span className="text-sm text-[#64748B]">Không tìm thấy file PDF trong kho lưu trữ.</span>
        </div>
      ) : (
        <div className="flex flex-col gap-3">
          {items.map((item) => (
            <MetadataCard key={item.data_path} item={item} onApply={handleApply} />
          ))}
        </div>
      )}
      ...
      {!loading && (
        <div className="flex items-center justify-between rounded-2xl border border-[#E2E8F0] bg-white px-6 py-4">
          ...
          <button
            ...
            onClick={() => {
            if (!allApplied) {
              toast.error(`Còn ${items.length - appliedCount} file chưa được áp dụng.`)
              return
            }
            ...
            style={allApplied ? { background: "linear-gradient(to right, #0052FF, #4D7CFF)", boxShadow: "0 4px 14px rgba(0,82,255,0.25)" } : {}}
          >
            Phân cụm tài liệu →
          </button>
        </div>
