...
export function ProcessStepFooter({
  ...
  const reviewWarningCount = Math.max(pendingCount, Number(warningCount) || 0)
  const showWarning = reviewWarningCount > 0
  const warningTone = Boolean(buildBlockedMessage) || showWarning
  const statusTone: WorkflowActionTone = warningTone
    ? "warning"
    : readyItems.length > 0
      ? "success"
      : "progress"

  const statusIcon = warningTone ? (
  ...
  const statusTitle = buildBlockedMessage
    ? "Chưa thể lập hồ sơ"
    : showWarning
  ...
  const statusDescription = buildBlockedMessage
    ? buildBlockedMessage
    : showWarning
