...
import { FileText, Eye, X, ChevronDown, ChevronRight, CheckCircle2, Loader2 } from "lucide-react"
...
function PdfPreviewModal({ filePath, metadata, onClose }: {
  filePath: string
      ...
      >
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          <div className="flex items-center gap-3">
            <div className="flex size-8 items-center justify-center rounded-lg shadow-[0_4px_14px_rgba(0,82,255,0.25)]"
              style={{ background: "linear-gradient(135deg, #0052FF, #4D7CFF)" }}>
              <FileText className="size-4 text-white" />
            </div>
            <div>
              <p className="text-sm font-semibold text-foreground">{filePath.split("/").pop()}</p>
        ...
        <div className="p-5">
          {metadata ? (
            ...
            <div className="flex items-center justify-center py-8 text-sm text-muted-foreground">
              <Loader2 className="mr-2 size-4 animate-spin" /> Đang tải metadata…
            </div>
          )}
        </div>
      </motion.div>
...
function ClusterGroupCard({ group, onExclude, onPreview }: {
  group: ClusterGroup
    ...
    <div className="overflow-hidden rounded-xl border border-border bg-card">
      <button onClick={() => setOpen((v) => !v)}
        ...
        <div className="flex items-center gap-2">
          <div className="flex size-6 items-center justify-center rounded-md text-[10px] font-bold text-primary-foreground shadow-[0_2px_8px_rgba(0,82,255,0.3)]"
            style={{ background: "linear-gradient(135deg, #0052FF, #4D7CFF)" }}>
              ...
              {group.files.map((filePath) => (
                <motion.div key={filePath}
                  ...
                  <div className="flex shrink-0 items-center gap-1">
                    <Button variant="outline" size="sm" onClick={() => onPreview(filePath)}
                      className="h-6 gap-1 px-2 text-[10px]">
                      <Eye className="size-3" /> Xem
                    </Button>
                    <Button variant="destructive" size="sm" onClick={() => onExclude(group.id, filePath)}
                      className="h-6 gap-1 px-2 text-[10px]">
...
export function ClusterPanel({
  groups, excludedFiles, metadataMap, clusterDone,
  clusterStatus, loadedCount, totalCount, onExclude,
}: ClusterPanelProps) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="rounded-xl border border-border bg-card p-3">
        ...
        <ScrollArea className="h-[360px]">
          <div className="flex flex-col gap-2 pr-1">
            {groups.map((group) => (
              <ClusterGroupCard key={group.id} group={group} metadataMap={metadataMap}
                onExclude={onExclude} onPreview={setPreviewFile} />
            ))}
            ...
            {excludedFiles.length > 0 && (
              <div className="rounded-xl border border-destructive/20 bg-destructive/5 p-3">
                ...
                {excludedFiles.map((f) => (
                  <p key={f} className="truncate font-mono text-[10px] text-destructive/70">
                    {f.split("/").pop()}
                  </p>
                ))}
              </div>
      ...
      <AnimatePresence>
        {previewFile && (
          <PdfPreviewModal filePath={previewFile} metadata={metadataMap[previewFile]}
