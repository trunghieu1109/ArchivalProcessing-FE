...
export function FolderNameForm({ node, onSave }: FolderNameFormProps) {
  ...
  const handleSave = () => {
    ...
    if (!soHoSo.trim()) {
      toast.error("Vui lòng nhập số hồ sơ.")
      return
    }
    if (!thoiHan.trim()) {
      toast.error("Vui lòng nhập thời hạn bảo quản.")
      return
    }
    onSave(node.id, {
      hoSoName: hoSoName.trim(),
      soHoSo: soHoSo.trim(),
      thoiHanBaoQuan: thoiHan.trim(),
    })
    setOpen(false)
  ...
  return (
    <div
      className={cn(
        ...
        isFilled
          ? "border-[#0052FF]/20 bg-[#0052FF]/[0.02]"
    ...
    >
      ...
      >
        <div className="flex items-center gap-2">
          {isFilled ? (
            <CheckCircle2 className="size-3.5 text-[#0052FF]" />
          ) : (
            <Edit2 className="size-3.5 text-[#94A3B8]" />
          )}
          <span
            className={cn(
              "text-xs font-medium",
              isFilled ? "text-[#0052FF]" : "text-[#64748B]"
            )}
          >
          ...
          {isFilled && (
            <span className="text-[10px] text-[#64748B]">
              · {node.soHoSo} · {node.thoiHanBaoQuan}
            </span>
          )}
        </div>
        {open ? (
              ...
              {[
                {
                  ...
                  value: hoSoName,
                  set: setHoSoName,
                  placeholder: "VD: Hồ sơ bổ nhiệm 2021",
                },
                {
                  label: "Số hồ sơ",
                  value: soHoSo,
                  set: setSoHoSo,
                  placeholder: "VD: 01/HS-2021",
                },
                {
                  label: "Thời hạn bảo quản",
                  value: thoiHan,
                  set: setThoiHan,
                  placeholder: "VD: 10 năm / Vĩnh viễn",
                },
              ].map(({ label, value, set, placeholder }) => (
                <div key={label} className="flex items-center gap-2">
                  <label className="w-32 shrink-0 text-[11px] font-medium text-[#64748B]">
                    {label}
                  </label>
                  <input
                    value={value}
