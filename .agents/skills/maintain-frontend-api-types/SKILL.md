---
name: maintain-frontend-api-types
description: "Maintain request, response, session, document, cluster, artifact, job, and publication types with runtime normalization. Use when working on TypeScript API type, response type, session type, document type, cluster type, job status."
---

# Maintain Frontend API Types
...
relevant paths from the manifest.Linked backend skills: `$maintain-archival-api-contracts`. Load them only when the server contract or behavior is in scope.
