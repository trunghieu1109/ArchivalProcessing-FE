---
name: manage-frontend-document-mutations
description: "Maintain document deletion and transfer previews, confirmation, progress, conflicts, visibility, refresh, and downstream warnings. Use when working on delete document UI, transfer dialog, deletion preview, transfer conflict, deleted visibility."
---

# Manage Frontend Document Mutations
...
Recommended overlays: `$manage-frontend-document-locks`, `$review-frontend-cross-step-changes`.
Linked backend skills: `$delete-archival-documents`, `$transfer-archival-documents`. Load them only when the server contract or behavior is in scope.
