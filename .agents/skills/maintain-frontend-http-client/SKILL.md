---
name: maintain-frontend-http-client
description: "Maintain base URL resolution, authentication headers, request encoding, response parsing, aborts, and normalized HTTP errors. Use when working on sessionApi.http, fetch wrapper, base URL, auth header, HTTP error, AbortController."
---

# Maintain Frontend Http Client
...
relevant paths from the manifest.Linked backend skills: `$maintain-archival-api-contracts`, `$configure-archival-runtime`. Load them only when the server contract or behavior is in scope.
