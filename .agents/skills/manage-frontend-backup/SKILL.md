---
name: manage-frontend-backup
description: "Maintain session-backup visibility, request handling, progress or result presentation, authorization feedback, and backup downloads. Use when working on session backup UI, backup button, backup visibility, backup API, backup download."
---

# Manage Frontend Backup
...
Recommended overlays: `$maintain-frontend-errors-downloads`, `$manage-frontend-auth`.
Linked backend skills: `$operate-archival-backup-export`. Load them only when the server contract or behavior is in scope.
