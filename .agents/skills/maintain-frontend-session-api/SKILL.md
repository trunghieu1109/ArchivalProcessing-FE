---
name: maintain-frontend-session-api
description: "Maintain session API facade, domain modules, imports, request helpers, exports, and capability boundaries. Use when working on sessionApi.ts, API module split, session API facade, API helper, public export."
---

# Maintain Frontend Session API
...
Recommended overlays: `$maintain-frontend-http-client`, `$maintain-frontend-api-types`.
Linked backend skills: `$maintain-archival-api-contracts`. Load them only when the server contract or behavior is in scope.
