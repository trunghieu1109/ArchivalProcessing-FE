---
name: manage-frontend-document-locks
description: "Maintain document edit-lock acquire, heartbeat, release, late-response handling, bulk-lock behavior, unload cleanup, and conflict feedback. Use when working on document edit lock, lock heartbeat, lock conflict, bulk edit locks, release lock."
---

# Manage Frontend Document Locks
...
Recommended overlays: `$maintain-frontend-errors-downloads`.
Linked backend skills: `$manage-document-edit-locks`. Load them only when the server contract or behavior is in scope.
