---
name: manage-frontend-folder-upload
description: "Maintain folder selection, manifest registration, transfer progress, pause, retry, cancellation, reconciliation, and global dock behavior. Use when working on folder upload UI, folder manifest, upload dock, folder completion, folder reconciliation."
---

# Manage Frontend Folder Upload
...
Recommended overlays: `$maintain-frontend-upload-managers`, `$maintain-frontend-polling`.
Linked backend skills: `$manage-folder-uploads`. Load them only when the server contract or behavior is in scope.
