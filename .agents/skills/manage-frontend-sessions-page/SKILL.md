---
name: manage-frontend-sessions-page
description: "Maintain session listing, pagination, open, delete visibility, coordinator assignment, status display, and empty or error states. Use when working on sessions page, session list, pagination, open session, assign coordinator, delete session."
---

# Manage Frontend Sessions Page
...
Recommended overlays: `$maintain-frontend-session-api`.
Linked backend skills: `$manage-archival-sessions`, `$manage-archival-access`. Load them only when the server contract or behavior is in scope.
