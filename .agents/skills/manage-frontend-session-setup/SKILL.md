---
name: manage-frontend-session-setup
description: "Maintain Step 1 session creation, archive metadata, plan and retention selection, dossier title catalog, staged inputs, and confirmation. Use when working on Step 1, session setup, archive metadata, plan upload selection, title catalog, upload confirmation."
---

# Manage Frontend Session Setup
...
Recommended overlays: `$manage-frontend-zip-upload`, `$manage-frontend-folder-upload`.
Linked backend skills: `$manage-archival-sessions`, `$manage-archival-uploads`, `$manage-dossier-title-catalog`. Load them only when the server contract or behavior is in scope.
