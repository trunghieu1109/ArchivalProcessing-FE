---
name: manage-frontend-artifacts
description: "Maintain finalization dispatch, polling, artifact freshness, metadata XLSX import, DOCX/XLSX preview, signed downloads, and output state. Use when working on finalize artifacts page, artifact preview, metadata XLSX import, artifact polling, signed artifact URL."
---

# Manage Frontend Artifacts
...
Recommended overlays: `$maintain-frontend-polling`, `$maintain-frontend-errors-downloads`.
Linked backend skills: `$generate-archival-artifacts`, `$import-dossier-metadata-xlsx`. Load them only when the server contract or behavior is in scope.
