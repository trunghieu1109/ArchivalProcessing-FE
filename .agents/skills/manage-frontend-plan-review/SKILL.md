---
name: manage-frontend-plan-review
description: "Maintain Step 2 plan progress, parsing, tree editing, strategy, save, activation, dirty state, and downstream configuration. Use when working on Step 2, plan review, folder tree, save plan, activate plan, plan strategy."
---

# Manage Frontend Plan Review
...
Recommended overlays: `$maintain-frontend-polling`, `$maintain-frontend-api-types`.
Linked backend skills: `$manage-archival-plans`, `$manage-retention-sources`. Load them only when the server contract or behavior is in scope.
