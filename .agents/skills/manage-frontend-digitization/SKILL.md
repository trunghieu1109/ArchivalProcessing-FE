---
name: manage-frontend-digitization
description: "Maintain Step 3 digitization start, status, progress, recovery, assignment controls, document listing, and processing actions. Use when working on Step 3, digitization UI, OCR progress, restart processing, batch assignment, review controls."
---

# Manage Frontend Digitization
...
Recommended overlays: `$maintain-frontend-polling`, `$manage-frontend-document-metadata`.
Linked backend skills: `$manage-archival-digitization`, `$manage-batching-assignment`. Load them only when the server contract or behavior is in scope.
