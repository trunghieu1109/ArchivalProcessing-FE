---
name: manage-frontend-document-metadata
description: "Maintain metadata cards, field normalization, edits, bulk actions, review, verification, OCR results, and manual-value ownership. Use when working on metadata card, edit document metadata, bulk metadata, review document, verify document, OCR results."
---

# Manage Frontend Document Metadata
...
Recommended overlays: `$manage-frontend-document-locks`, `$maintain-frontend-api-types`.
Linked backend skills: `$manage-document-metadata`. Load them only when the server contract or behavior is in scope.
